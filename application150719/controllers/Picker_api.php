...
class Picker_api extends CI_Controller
{
            ...
            if($this->Model->get_record('pd_person', array('id' => $picker_id))){
                ...
                if ($imageresult) {
                    if ($this->Model->update('pd_person', array('name' =>$name,'Address'=>$address,'zip_code'=>$zipCode,'image'=>$uploadpath ), array('id' => $picker_id ))) {
                        $userDetail = $this->Model->get_record('pd_person', array('id' => $picker_id));
                        $response["error"] = false;
                ...
                if ($this->Model->update('pd_person', array('name' =>$name,'Address'=>$address,'zip_code'=>$zipCode ), array('id' => $picker_id ))) {
                    $userDetail = $userDetail = $this->Model->get_record('pd_person', array('id' => $picker_id));
                    $response["error"] = false;
                    $response["message"] = "Profile Successfully Updated";
                    $response["userDetail"] = $userDetail;
                } else {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if($api_key != ''){
                ...
                if ($store_not_match == true) {
                    ...
                    $pickerStatus    = $this->Model->get_selected_data('picker_order_status', 'pd_person', array('id' => $picker_id));
                    $pickerStatusMsg = $this->getPickerStatusMsg($pickerStatus);
                    if ($pickerStatus == 0) {
                        
                        if (!$this->Model->get_selected_data('id', 'pd_order', array('id' => $orderids)))
                         {
                            if ($this->Model->add('pd_order', $insertArray)) {
                                ...
                                $this->Model->update('order_table', array('status' => $orderStatus), array('order_id' => $order_id));
                                $this->Model->update('pd_person', array('picker_order_status' => $status), array('id' => pickerid));
                                $response["error"]   = false;
    ...
    {    
        ...
        if($api_key != ''){
            ...
            */
            ...
            $picker_non_delievered_orders     = $this->Model->get_selected_data('order_id', 'pd_order', array('picker_id' => $picker_id,'status' => 0));
            ...
            if ($picker_non_delievered_orders_num > 0) {
                ...
                foreach ($picker_non_delievered_orders_num as $order_id) {
                    $order_details = $this->Model->get_record('order_table', array('order_id' => $order_id['order_id'],'status' => 'active'));
                ...
                if ($count_of_cancelled_orders == $picker_non_delievered_orders_num) {
                    ...
                    $this->Model->update('pd_person', array('picker_order_status' => 0), array('id' => $userid));
                }
            }
                ...
                if ($pickerOrders->num_rows > 0) {
                    ...
                    if ($itemInfo->num_rows > 0) {
                        foreach ($results as $result) {
                            ...
                            $result['sub_name']             = $db->getSubCategoryNameForItem($result['item_id'], $store_id);
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($pickerOrders) {
                    ...
                    foreach ($pickerOrders as $row) {
                        ...
                        $singleOrderItemCount = $this->Model->get_record('ordered_item', array('order_id' => $mobileno,'status' => 1 ));
                    ...
                    if ($totalItemCount == $item_count) {
                        ...
                        $isUpdatePickerStatus = $this->Model->update('pd_person', array('picker_order_status' => 3), array('id' => $picker_id ));
                        $pickerStatus         = $this->Model->get_selected_data('picker_order_status', 'pd_person', array('id' => $picker_id));
                        ...
                        $isUpdateOrderStatus = $this->Model->update('order_table', array('status' => $status), array('order_id' => $orderIds));
					...
					if(Count($orderInfo) > 0){
						...
						if($currentOrderStatus != $order_status){
							...
							if ($isspayment) {
                                ...
                                $issendmail = send_mail($bi_email,"Your Go2Gro order (".$orderid.") has been placed !",$mail_temp);

								$db->updateSingleOrderStatus($order_id, $order_status);
                ...
                $orderInfo = $this->Picker_model->getOrderInfoByOrderId($order_id);
                    if(count($orderInfo) > 0){
                        ...
                        if($currentOrderStatus != $status) {
                            ...
                            $db->updateSingleOrderStatus($order_id, $status);
                            ...
                            $picker_status = $this->Model->get_selected_data('picker_order_status', 'pd_person', array('id' => $picker_id));//$db->getPickerStatus($picker_id);
                            $response['error'] = false;
    ...
    public function completeOrderAlternateSuggest()
    {
        ...
        if ($this->form_validation->run() == true) 
        {
            ...
            $finalArray = array();
            $currenttime = time();
            $db->getconction()->autocommit(FALSE);// change according to ci
            $proceed_flag = true;
            $store_id = -1;
            foreach($orderidsArry as $orderid) {
                ...
                if($updatecartItemStatus) {
                    ...
                    if(!$updateOrderEditStatusAlternateTime){
                        $db->getconction()->rollBack();// change according to ci
                        $proceed_flag = false;
                        break;
                    }
                } else {
            ...
            $db->getconction()->commit();// change according to ci
            $db->getconction()->autocommit(TRUE);// change according to ci
            if($proceed_flag) {
                foreach ($orderidsArry as $orderid) {
            ...
            Thank you for your order! <br><br> Unfortunately due to the unavailability of some product(s), your order needs to be reviewed for alternative options. <br><br><strong> Please click on the following link to review more options for the unavailable product(s).<strong> <br><br> " . $link . " <br><br> We apologize for this inconvenience, however endeavour to cater for your every need.";
                           ...
                           $mail_msg = $this->load->view('go2gro_web/template/Picker_orderalternetlink',$udata,true);
                            $issendmail = send_mail($email,$subject,$msglink);
                            $ordmessagelink = urlencode($msglink);
                            $sendsms = sendsms($mobilenumber, $ordmessagelink);
                            ...
                            if (count($resp) > 0) {
                                ...
                                manage_notification("0",$notifyuseridarray,$orderid,ORDER_SEND_ALTERNATIVE,$date,$timedate,ORDER_TAG,"NA","NA");
                            }
    ...
    public function addorderstatus()
    {
        ...
        if($user){
            if ($status == 4 || $status == 3) {
                $issmssend = sendstatussms($usermobile, $username, $status, $order_id);// change according to ci
            }
            ...
            $notifyuseridarray=array();
            $respx=$db->getgcmdata($order_user_id);//change according to ci
            if (count($respx) > 0) {
                ...
                manage_notification("0",$notifyuseridarray,$order_id,$tag,$date,$timedate,ORDER_TAG,"NA","NA");//change according to ci
            }
            return true;
        }else{
            return false;
