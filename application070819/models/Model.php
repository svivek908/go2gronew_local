...
Class Model extends MY_Model{
   ...
   //---------All Order ccount----
    ...
    public function getitemsuggetionbystr($str, $store_id, $deviceType = NULL){
        ...
        $itemlink_table =   STORE_PREFIX.$store_id.'_'.ITEMLINK_TABLE;
        if ($deviceType == "ios") {
             $this->db->select("il.subcat_id,a.item_id,a.item_name,c.id from ((SELECT item_id,CONCAT(`item_name`, ' ', `item_size`) as item_name FROM `".$item_table."` WHERE `item_name` like '$str%' and `item_status`='0') UNION (SELECT item_id,CONCAT(`item_name`, ' ', `item_size`) as item_name FROM `".$item_table."` WHERE CONCAT(`item_name`, ' ', `item_size`) like '%$str%' and `item_status`='0') ) AS a join ".$itemlink_table." il on a.item_id=il.item_id join subcategory sub on sub.sub_id=il.subcat_id join category c on c.id =sub.cat_id AND c.id NOT IN($ids) AND il.status=0 GROUP by a.item_id order by c.id asc LIMIT 0,10");
        } else {
              $this->db->select(" il.subcat_id,a.item_id,a.item_name,c.id from ((SELECT item_id,CONCAT(`item_name`, ' ', `item_size`) as item_name FROM `".$item_table."` WHERE CONCAT(`item_name`, ' ', `item_size`) like '$str%' and `item_status`='0') UNION (SELECT item_id,CONCAT(`item_name`, ' ', `item_size`) as item_name FROM `".$item_table."` WHERE `item_name` like '%$str%' and `item_status`='0') ) AS a join ".$itemlink_table." il on a.item_id=il.item_id join subcategory sub on sub.sub_id=il.subcat_id join category c on c.id =sub.cat_id AND il.status=0 GROUP by a.item_id order by c.id asc LIMIT 0,10");
        }
        $query = $this->db->get();
        if ($query->num_rows() > 0) 
        {
    ...
    public function getitemsuggetion($str, $firststr, $seconstr, $store_id, $deviceType = NULL)
    {
        ...
        if ($deviceType == "ios") {
            $this->db->select("il.subcat_id,a.item_id,a.item_name,c.id from ((SELECT item_id,CONCAT(`item_name`, ' ', `item_size`) as item_name FROM `".$item_table."` WHERE (CONCAT(`item_name`, ' ', `item_size`) like '$str%' or CONCAT(`item_name`, ' ', `item_size`) like '$firststr%' or CONCAT(`item_name`, ' ', `item_size`) like '$seconstr%') and `item_status`='0') UNION (SELECT item_id,CONCAT(`item_name`, ' ', `item_size`) as item_name FROM `".$item_table."` WHERE (CONCAT(`item_name`, ' ', `item_size`) like '%$str%' or CONCAT(`item_name`, ' ', `item_size`) like '%$firststr%' or CONCAT(`item_name`, ' ', `item_size`) like '%$seconstr%' ) and `item_status`='0'))
            ...
            $this->db->select("il.subcat_id,a.item_id,a.item_name,c.id from ((SELECT
    ...
    public function getsuggetionitemscountbystr($str, $store_id, $deviceType = NULL)
    {
        ...
        if ($deviceType == "ios") {
            $this->db->select(" a.item_id,a.item_name,a.`item_sdesc`,a.`item_fdesc`,a .`item_price`,a.`item_status`,a.`Sales_tax`,a.rating_average,a.item_image,il.subcat_id,c.id
        from (
